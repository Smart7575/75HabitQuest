...
export const TasksPage: React.FC = () => {
  ...
  const handleOpenEditTask = (e: React.MouseEvent, task: Task) => {
    ...
    setTaskForm({ 
      ...
      startDate: task.startDate 
        ...
        : (task.createdAt ? format(new Date(task.createdAt), 'yyyy-MM-dd') : format(new Date(), 'yyyy-MM-dd')),
      days: task.days || [] 
    });
  ...
  const handleSubmitTask = async (e: React.FormEvent) => {
    ...
    const taskData = { 
      ...
      startDate: taskForm.startDate ? new Date(`${taskForm.startDate}T00:00:00`).getTime() : Date.now()
    };
  ...
  return (
    <div className="pb-24 w-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-gray-800">{t.tasks}</h1>
          <p className="text-gray-500 text-sm">{t.planRoutine}</p>
        </div>
        <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">
          <button type="button" onClick={() => setIsCategoryModalOpen(true)} className="flex-1 sm:flex-none bg-white border border-slate-200 text-slate-600 px-3.5 py-2 rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center gap-2 hover:bg-slate-50 transition-all">
            <FolderPlus className="w-4 h-4 sm:w-5 sm:h-5" /> {t.categories}
          </button>
          <button type="button" onClick={() => { resetTaskForm(); setIsModalOpen(true); }} className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center gap-2 shadow-lg shadow-blue-200 transition-all">
            <Plus className="w-4 h-4 sm:w-5 sm:h-5" /> {t.createTask}
          </button>
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-8 w-full">
        <div className="p-3 sm:p-4 border-b border-gray-50 bg-gray-50/50 flex flex-wrap gap-4 items-center">
        ...
        <div className="w-full overflow-x-auto custom-scrollbar">
          <table className="w-full text-left min-w-[540px] sm:min-w-full border-collapse">
            <thead className="bg-slate-50/70 border-b border-slate-100 text-left text-[10px] sm:text-xs uppercase text-slate-400 font-bold tracking-wider">
              <tr>
                <th className="px-3 sm:px-5 py-3 min-w-[140px]">{t.taskName}</th>
                <th className="px-2 sm:px-4 py-3 w-28 shrink-0">{t.type}</th>
                <th className="px-2 sm:px-4 py-3 w-40 sm:w-48 shrink-0">{t.schedule}</th>
                <th className="px-2 sm:px-4 py-3 w-20 shrink-0 text-center sm:text-left">{t.points}</th>
                <th className="px-3 sm:px-5 py-3 w-20 shrink-0 text-right">{t.actions}</th>
              </tr>
            </thead>
              ...
              {groupedTasks.map(([category, catTasks]) => (
                <React.Fragment key={category}>
                  <tr className="bg-slate-100/60">
                    <td colSpan={5} className="px-3 sm:px-5 py-2 text-[10px] font-black text-slate-500 uppercase tracking-widest">
                      {category}
                    </td>
                  </tr>
                    ...
                    <tr key={task.id} className="hover:bg-slate-50/70 transition-colors group">
                      <td className="px-3 sm:px-5 py-2.5 sm:py-3">
                        <div className="flex items-center gap-2">
                          <span 
                            className="font-semibold text-slate-800 text-xs sm:text-sm leading-tight break-words" 
                            title={task.name}
                          >
                            {task.name}
                          </span>
                          {task.description && (
                            <button 
                              ...
                              className="p-1 hover:bg-blue-50 text-blue-500 rounded-full transition-colors shrink-0"
                      ...
                      <td className="px-2 sm:px-4 py-2.5 sm:py-3 shrink-0">
                        <span className={`px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-bold uppercase whitespace-nowrap inline-block
                      ...
                      <td className="px-2 sm:px-4 py-2.5 sm:py-3 shrink-0">
                        {task.frequency && task.frequency > 0 ? (
                          <span className="text-[10px] sm:text-xs font-bold text-indigo-600 whitespace-nowrap">{t.every} {task.frequency} {t.days}</span>
                        ) : (
                          <div className="flex gap-1">
                            {[1,2,3,4,5,6,0].map(d => (
                              <span key={d} className={`w-5 h-5 flex items-center justify-center rounded text-[9px] font-bold shrink-0 ${task.days?.includes(d) ? 'bg-blue-600 text-white shadow-xs' : 'bg-slate-100 text-slate-400'}`}>
                      ...
                      <td className="px-2 sm:px-4 py-2.5 sm:py-3 font-bold text-slate-700 text-xs sm:text-sm whitespace-nowrap text-center sm:text-left">
                        {task.points} XP
                      </td>
                      <td className="px-3 sm:px-5 py-2.5 sm:py-3 text-right shrink-0">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleSubmitTask} className="p-6 space-y-4 overflow-y-auto custom-scrollbar">
              <div>
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-48 sm:w-56 max-w-full px-3.5 py-2 rounded-xl border border-gray-300 bg-white text-slate-900 font-semibold focus:ring-2 focus:ring-blue-500/20 focus:outline-none text-sm" 
                />
              </div>
